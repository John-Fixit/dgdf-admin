    ...
    try {
      for (const [index, item] of pending.entries()) {
        const payload: GalleryUploadPayload = {
          ...
          description: `${defaults.status} · sort ${defaults.sortOrder + index}`,
          ...
          <Controller
            ...
            render={({ field }) => (
              <Input
                ...
                onValueChange={(value) =>
                  field.onChange(Number.isFinite(Number(value)) ? Number(value) : 0)
                }
                classNames={{
                  label: 'text-sm font-semibold text-primary',
                  inputWrapper:
          ...
          >
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                className="text-xs font-semibold text-slate-400 transition-colors hover:text-error"
              >
                Clear all
              </button>
            </div>
