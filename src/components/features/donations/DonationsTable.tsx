...
import { recordDonationExport } from "@/lib/api";
...
export function DonationsTable(): React.ReactElement {
  ...
  const { user } = useAuth();
  const canExport = can(user?.role, "exportData");
  ...
  async function handleExport(): Promise<void> {
    ...
    try {
      await recordDonationExport();
    } catch {
      // Export already downloaded; audit log failure is non-blocking
    }
  }
              ...
              {pageItems.length === 0 ? (
                ...
                pageItems.map((donation, index) => (
                  ...
                  >
                    ...
                    <td className="px-6 py-3 font-semibold text-primary sm:px-8">
                      {formatCurrency(donation.amount, donation.currency)}
                    </td>
                    <td className="px-6 py-3 text-slate-500 sm:px-8">
